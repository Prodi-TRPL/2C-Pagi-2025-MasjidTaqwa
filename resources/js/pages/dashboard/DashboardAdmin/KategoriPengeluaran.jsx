...
import { FiEdit2, FiTrash2, FiPlus, FiCheck, FiX, FiInfo, FiFilter, FiList, FiTag, FiPieChart, FiAlertCircle, FiLock } from "react-icons/fi";
    ...
    try {
      ...
      if (res.data && res.data.length > 0) {
        setStats(prevStats => ({
          ...
          // We'll update mostUsedCategory and unusedCategories when we have that data
        }));
      }
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-green-100 opacity-90 mt-1">Kelola kategori pengeluaran untuk laporan keuangan</p>
            </div>
          </div>
        </div>
      </div>
          ...
          >
            ...
            <div className="p-6">
              <div className="flex items-center">
                ...
                <div>
                  <p className="text-sm font-medium text-gray-500">Kategori Terbanyak</p>
                  <p className="text-2xl font-bold text-gray-800">{stats.mostUsedCategory.name}</p>
                </div>
              </div>
            </div>
          </motion.div>
            ...
            <div className="p-6">
              <div className="flex items-center">
                <div className="p-3 rounded-full bg-yellow-100 mr-4">
                  <FiPieChart className="h-6 w-6 text-yellow-600" />
                </div>
                <div>
                  <p className="text-sm font-medium text-gray-500">Penggunaan Kategori</p>
                  <p className="text-2xl font-bold text-gray-800">{isNaN(100 - (stats.unusedCategories / stats.totalCategories * 100)) ? 0 : 100 - (stats.unusedCategories / stats.totalCategories * 100)}%</p>
                </div>
              </div>
            </div>
          </motion.div>
                  ...
                  <form onSubmit={handleTambah} className="space-y-4">
                    <div>
                      ...
                      <motion.div
                        ...
                        animate="blur"
                        variants={{
                          focus: { scale: 1, borderColor: '#59B997', boxShadow: '0 0 0 3px rgba(89, 185, 151, 0.2)' },
                          blur: { scale: 1, borderColor: '#e5e7eb', boxShadow: 'none' }
                        }}
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredKategori.length > 0 ? (
                    filteredKategori.map((item) => (
                      <motion.tr 
                        key={item.kategori_pengeluaran_id || item.id} 
                        className="hover:bg-gray-50"
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        transition={{ duration: 0.3 }}
                      >
                        <td className="px-6 py-4 text-sm font-medium text-gray-800">{item.kategori_pengeluaran_id || item.id}</td>
                        <td className="px-6 py-4 text-sm text-gray-600">
                          {editingId === (item.kategori_pengeluaran_id || item.id) ? (
                            <motion.div
                            ...
                            {editingId === (item.kategori_pengeluaran_id || item.id) ? (
                              <>
                                <motion.button
                                  ...
                                  whileTap={{ scale: 0.9 }}
                                  onClick={() => handleEdit(item.kategori_pengeluaran_id || item.id)}
                              ...
                              <>
                                <motion.button
                                  ...
                                  whileTap={{ scale: 0.9 }}
                                  onClick={() => {
                                    setEditingId(item.kategori_pengeluaran_id || item.id);
                                ...
                                <motion.button
                                  whileHover={{ scale: 1.1 }}
                                  whileTap={{ scale: 0.9 }}
                                  onClick={() => handleHapus(item.kategori_pengeluaran_id || item.id, item.nama_kategori)}
